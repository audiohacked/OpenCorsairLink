...
void led_suboptions_parse(char *subopts, struct led_control *settings)
{
    ...
    case SUBOPTION_LED_COLORS: /* led color */
        ...
        while( token != NULL )
        {
            ...
            sscanf(token, "%02hhX%02hhX%02hhX,",
    ...
    case SUBOPTION_LED_TEMPERATURE: /* led warning temperatures */
        ...
        while( token != NULL )
        {
            if (ii == 3) break;
            msg_debug("Found Color %d: %s\n", ii, token);
            if (ii==0) sscanf(token, "%hd", &settings->temperatures[0]);
            if (ii==1) sscanf(token, "%hd", &settings->temperatures[1]);
            if (ii==2) sscanf(token, "%hd", &settings->temperatures[2]);
            ++ii;
            token = strtok(NULL, ":");
        }
        break;
