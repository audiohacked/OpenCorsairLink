...
int corsairlink_coolit_device_id(struct corsair_device_info *dev, struct libusb_device_handle *handle,
            uint8_t *device_id)
{
    int rr;
    uint8_t response[64];
...
int corsairlink_coolit_name(struct corsair_device_info *dev, struct libusb_device_handle *handle,
...
int corsairlink_coolit_vendor(struct corsair_device_info *dev, struct libusb_device_handle *handle,
            char *name, uint8_t name_size)
{
...
int corsairlink_coolit_product(struct corsair_device_info *dev, struct libusb_device_handle *handle,
            char *name, uint8_t name_size)
{
    snprintf(name, name_size, "%s", dev->name);
    return 0;
            ...
            uint8_t firmware_size)
{
    ...
    snprintf(firmware, firmware_size, "%d.%d.%d", (response[3] & 240) >> 4, (response[3] & 15), response[2]);
