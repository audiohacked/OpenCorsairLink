...
int corsairlink_commanderpro_get_led_setup_mask(struct corsair_device_info *dev,
			struct libusb_device_handle *handle, uint8_t led_channel)
{
	int rr;
	uint8_t response[16];
	uint8_t commands[64] ;
...
int corsairlink_commanderpro_set_led_rgb_colour(struct corsair_device_info *dev,
			struct libusb_device_handle *handle, uint8_t led_channel, struct color *color_led)
{
	int rr;
	uint8_t response[16];
	uint8_t commands[64] ;
...
int corsairlink_commanderpro_set_led_config(struct corsair_device_info *dev, 
			struct libusb_device_handle *handle, uint8_t led_channel,
            uint8_t strip_count, uint8_t led_type, uint8_t mode,
            uint8_t speed, uint8_t direction, struct color led_color,
            struct led_temperatures led_temperature)
{
	...
	commands[0x03] = led_type; // 0x0A = LED Strip, 0x0C = HD Fan, 0x01 = SP Fan, 0x04 = ML Fan
	commands[0x04] = mode;
    commands[0x05] = speed;
