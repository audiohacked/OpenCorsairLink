...
#define COOLIT_FAN_TABLE_QUIET(x) struct fan_table x = {\
    .t1 = 0x14, .t2 = 0x20, .t3 = 0x28, .t4 = 0x32, .t5 = 0x37, .t6 = 0x3c,\
    .s1 = 0x19, .s2 = 0x27, .s3 = 0x32, .s4 = 0x4b, .s5 = 0x5a, .s6 = 0x64,\
}

#define COOLIT_FAN_TABLE_EXTREME(x) struct fan_table x = {\
...
int corsairlink_coolit_fan_print_mode(uint8_t mode,
            uint16_t data,
            char *modestr,
