...
struct corsair_device_driver
{
    ...
    /** protocol functions */
    int ( *name )( struct corsair_device_info* dev,
                   struct libusb_device_handle* handle, char* name,
                   uint8_t name_size );
    int ( *vendor )( struct corsair_device_info* dev,
                     struct libusb_device_handle* handle, char* name,
                     uint8_t name_size );
    int ( *product )( struct corsair_device_info* dev,
                      struct libusb_device_handle* handle, char* name,
                      uint8_t name_size );
    ...
    int ( *temperature )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle,
                          uint8_t sensor_select, double* temp );
    int ( *tempsensorscount )( struct corsair_device_info* dev,
                               struct libusb_device_handle* handle,
                               uint8_t* temperature_sensors_count );

    struct led_functions
    {
        int ( *static_color )( struct corsair_device_info* dev,
                               struct libusb_device_handle* handle,
                               struct led_control* ctrl );
        int ( *blink )( struct corsair_device_info* dev,
                        struct libusb_device_handle* handle,
                        struct led_control* ctrl );
        int ( *color_pulse )( struct corsair_device_info* dev,
                              struct libusb_device_handle* handle,
                              struct led_control* ctrl );
        int ( *color_shift )( struct corsair_device_info* dev,
                              struct libusb_device_handle* handle,
                              struct led_control* ctrl );
        int ( *rainbow )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle,
                          struct led_control* ctrl );
        int ( *temperature )( struct corsair_device_info* dev,
                              struct libusb_device_handle* handle,
    ...
    {
        int ( *count )( struct corsair_device_info* dev,
                        struct libusb_device_handle* handle,
                        uint8_t* fan_count );
        int ( *profile )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle, uint8_t selector,
                          uint8_t* profile, uint16_t* data );
        int ( *custom )( struct corsair_device_info* dev,
                         struct libusb_device_handle* handle, uint8_t selector,
                         struct fan_table* custom_profile );
        int ( *speed )( struct corsair_device_info* dev,
                        struct libusb_device_handle* handle, uint8_t selector,
    ...
    {
        int ( *supply_voltage )( struct corsair_device_info* dev,
                                 struct libusb_device_handle* handle,
                                 double* volts );
        int ( *total_wattage )( struct corsair_device_info* dev,
                                struct libusb_device_handle* handle,
                                double* watts );
        int ( *sensor_select )( struct corsair_device_info* dev,
                                struct libusb_device_handle* handle,
                                uint8_t sensor_select );
        int ( *voltage )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle,
                          uint8_t sensor_select, double* volts );
        int ( *amperage )( struct corsair_device_info* dev,
                           struct libusb_device_handle* handle,
                           uint8_t sensor_select, double* amps );
        int ( *wattage )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle,
    ...
    {
        int ( *powered )( struct corsair_device_info* dev,
                          struct libusb_device_handle* handle, uint32_t* v32 );
        int ( *uptime )( struct corsair_device_info* dev,
                         struct libusb_device_handle* handle, uint32_t* v32 );
    } psu_time;
